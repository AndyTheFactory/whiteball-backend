...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(['company_id'], ['companies.id'], ),
        sa.ForeignKeyConstraint(['matched_with_reference_id'], ['whiteball_packaging_items.id'], ),
        sa.PrimaryKeyConstraint('id'),
    )
    ...
    op.create_index('ix_product_packaging_associations_packaging_item_id', 'product_packaging_associations', ['packaging_item_id'])
