...
async def list_packaging_items(
    ...
    if search:
        stmt = stmt.where(
            (PackagingItem.name.ilike(f"%{search}%")) |
            (PackagingItem.material.ilike(f"%{search}%"))
        )
    ...
    count_stmt = select(func.count()).select_from(PackagingItem).where(
        PackagingItem.company_id == current_user.company_id
    )
...
async def delete_packaging_item(
    ...
    stmt = select(ProductPackagingAssociation).where(
        ProductPackagingAssociation.packaging_item_id == item_id
    )
...
@router.post("/products/{product_id}/packaging", response_model=PackagingAssociationResponse, status_code=status.HTTP_201_CREATED)
async def associate_packaging_with_product(
    ...
    if association_data.packaging_item:
        ...
        new_item = PackagingItem(
            company_id=current_user.company_id,
            **association_data.packaging_item.model_dump()
        )
        db.add(new_item)
        db.flush()
    ...
    stmt = select(PackagingItem).where(
        (PackagingItem.id == packaging_item_id) &
        (PackagingItem.company_id == current_user.company_id)
    )
...
async def update_product_packaging_association(
    ...
    stmt = select(ProductPackagingAssociation).where(
        ProductPackagingAssociation.id == association_id
    )
